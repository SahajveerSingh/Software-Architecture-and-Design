...
class MainWindow(tk.Tk):
    ...
    def _show_statistics(self):
        self._clear_content()
        try:
            from ui.statistics_ui import StatisticsPanel
            StatisticsPanel(
                self._content,
                catalogue_manager=self.catalogue_manager,
            ).pack(fill="both", expand=True)
            ...
            frame = tk.Frame(self._content, bg=BG, padx=28, pady=24)
            frame.pack(fill="both", expand=True)
            tk.Label(frame, text=f"Error loading statistics: {e}",
                     bg=BG, font=FONT_BODY, fg="red").pack()

    def _show_profile(self):
        try:
            from ui.auth_ui import ProfileWindow
    ...
    def _show_update_account(self):
        try:
            from ui.auth_ui import UpdateAccountWindow
